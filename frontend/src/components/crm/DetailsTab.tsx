  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <div className="shrink-0 px-3 py-2 border-b border-border flex items-center gap-2 min-w-0 bg-surface-sunken">
        ...
        {schema?.can_write ? <Button size="sm" variant="primary" disabled={!dirty} onClick={async () => { await actions.updateRecord(refr, draft); setDraft({}); }}><Save />Save</Button> : <span className="text-xs text-ink-3 shrink-0" title="Customer, Supplier and Employee records can be edited here only by Excom Admins">Read-only here</span>}
      </div>
