"""One Source list: every existing Lead Source (v15) / UTM Source (v16) without a matching Excom Intake Source
...
def execute():
	...
	have = {frappe.db.get_value("Excom Intake Source", n, "source_name") for n in frappe.get_all("Excom Intake Source", pluck="name")}
	n = 0
	for name in frappe.get_all(target, pluck="name"):
		...
		doc = frappe.get_doc({"doctype": "Excom Intake Source", "source_name": name, "source_type": stype, "enabled": 1, "company": company, "sla_first_response": 0})
