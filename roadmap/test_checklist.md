...
| One Source list: saving an Excom Source creates the Lead Source / UTM Source row; a manual lead with a source gets `intake_source` + attribution stamped; existing Lead Sources became Manual/Channel rows via `unify_sources` (26 on the dev copy) | S | ✅ test_intake_source_mirrors_attribution_and_stamps_leads |
| Rename Excom Intake Source / Log / Field Map → Excom Source / Source Log / Source Field Map via `rename_intake_to_source` (DocType rename moves tables + rewrites Link options); campaign/medium defaults per source stamped through crm_compat (Campaign on v15, UTM Campaign/Medium on v16) | S | ✅ migrate + 40 tests on the dev copy |
| WhatsApp diagnostics: Admin → WhatsApp templates → Diagnose accounts reports token stored / decryptable, WABA id set, and what Meta answers for the phone, the WABA and the template list, per account | S | ✅ run on the dev copy (correctly reports undecryptable tokens after the key change) |
| Template fetch compared line by line with shridarpatil/frappe_whatsapp: same endpoint, headers and hook-free upsert; ours adds paging, per-WABA grouping and error reporting | S | ✅ 2026-09-04 |
| Business Portfolio id in Business Account ID: Meta answers `(#100) Tried accessing nonexisting field (message_templates)`; sync resolves the WABA under it (owned_/client_whatsapp_business_accounts), corrects the account and retries; Diagnose offers the real WABA as a one-click fix | P1 | ✅ test_business_portfolio_id_is_healed_to_the_real_waba |
| One account with a missing / undecryptable token no longer aborts the sync for the others | P1 | ✅ credentials resolved inside the per-account try |
| Graph API v26 reviewed against Meta's changelog: no change to message_templates or WhatsApp Business Management; removed legacy protocol features (`pretty`, `debug`, `date_format`, root `?ids=`, ETag) are not used anywhere in excom | S | ✅ 2026-09-04 |
